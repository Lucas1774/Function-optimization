...
class DataSourceHandler:
    ...
    def parse_next_gen(variant: str, input_file: Dict[str, pd.DataFrame], current: pd.DataFrame) -> pd.DataFrame:
        constants = Constants()
        ...
        if variant == "variante-2":
            best = table_df.sort_values(by=table_df.columns[-1], ascending=False).head(5)
            table_df = pd.concat([table_df, best])

        table_df.insert(
            0, "individuo", range(1, len(table_df) + 1), True
        )
